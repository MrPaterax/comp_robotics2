...
class RGBDataset(Dataset):
    def __init__(self, dataset_dir, has_gt):
        ...
        self.dataset_length = len([f for f in os.listdir(dir) if not f.startswith('.')])
    ...
    def __getitem__(self, idx):
        ...
        rgb_dir = self.dataset_dir + 'rgb/' + str(idx) + '_rgb.png'
        rgb_img = self.transform(image.read_rgb(rgb_dir))
        gt_mask = None
        sample = None
        if self.has_gt is False:
            sample = {'input': rgb_img}
        else:
            gt_dir = self.dataset_dir + 'gt/' + str(idx) + '_gt.png'
            gt_mask = image.read_mask(gt_dir)
            sample = {'input': rgb_img, 'target': torch.LongTensor(gt_mask)}
        return sample
